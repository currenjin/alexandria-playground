...
/**
 * 릴레이 감시 지표 — age of oldest message(가장 오래된 미발행 이벤트의 나이) = now() - min(created_at) WHERE published_at IS NULL.
...
public class RelayLagMonitor {
    ...
    public void checkLag() {
        ...
        if (ageMs != null && ageMs > warnAgeMs) {
            log.warn("age of oldest message {}ms (> {}ms) — 릴레이 지연/정지 의심", ageMs, warnAgeMs);
        }
    }
}
