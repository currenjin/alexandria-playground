...
/**
 * 릴레이 감시 지표 (§7.1.4) — "미발행 최고령 나이" = now() - min(created_at) WHERE published_at IS NULL.
 * 릴레이가 멈추면 발행 0인데 에러도 안 나므로, 이 나이 증가로만 이상을 감지한다(건수는 피크에 정상적으로도 높음).
 */
@Component
public class RelayLagMonitor {
    ...
    public void checkLag() {
        ...
        if (ageMs != null && ageMs > warnAgeMs) {
            log.warn("outbox 미발행 최고령 나이 {}ms (> {}ms) — 릴레이 지연/정지 의심 (§7.1.4)", ageMs, warnAgeMs);
        }
    }
}
