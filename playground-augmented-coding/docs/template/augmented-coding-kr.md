항상 plan.md의 지시사항을 따르세요. 내가 "go"라고 말하면, plan.md에서 다음 표시되지 않은 테스트를 찾아 테스트를 구현한 다음, 그 테스트를 통과시키기 위한 최소한의 코드만 구현하세요.
...
- 테스트를 통과시키기 위해 필요한 최소한의 코드를 구현하세요
...
- 새 기능을 위해 사이클을 반복하세요
- 결함을 수정할 때는 먼저 API 수준의 실패하는 테스트를 작성하고, 그 다음 문제를 재현하는 가장 작은 테스트를 작성한 다음, 두 테스트를 모두 통과시키세요.

# TIDY FIRST 접근법

- 모든 변경을 두 가지 유형으로 구분하세요:
  1. 구조적 변경: 동작을 변경하지 않고 코드를 재배치 (이름 변경, 메서드 추출, 코드 이동)
  2. 행동적 변경: 실제 기능을 추가하거나 수정
- 구조적 변경과 행동적 변경을 같은 커밋에 섞지 마세요
- 둘 다 필요할 때는 항상 구조적 변경을 먼저 하세요
- 구조적 변경이 동작을 바꾸지 않았는지 테스트를 전후로 실행해 검증하세요
...
- 다음 조건에서만 커밋하세요:
  ...
  4. 커밋 메시지가 구조적 변경인지 행동적 변경인지 명확히 명시
...
1. 기능의 작은 부분에 대한 간단한 실패하는 테스트를 작성
2. 통과시키기 위한 최소한의 코드를 구현
3. 테스트를 실행하여 통과 확인 (Green)
4. 필요한 구조적 변경을 수행 (Tidy First), 각 변경 후 테스트 실행
5. 구조적 변경을 별도로 커밋
6. 다음 작은 기능 증분을 위한 또 다른 테스트 추가
7. 기능이 완성될 때까지 반복, 행동적 변경은 구조적 변경과 별도로 커밋

이 프로세스를 정확히 따르세요. 항상 빠른 구현보다 깔끔하고 잘 테스트된 코드를 우선시하세요.

항상 한 번에 하나의 테스트만 작성하고, 실행시킨 다음, 구조를 개선하세요. 매번 모든 테스트를 실행하세요 (장시간 실행되는 테스트 제외).
